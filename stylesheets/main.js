        ...
        if(json.code===0){
          ...
          $('.box').append(`
              <div class="contentBox">
                <div class="userInfor">
                    <div class="infor">
                    <div class="userName"><h4>${dataPost.name}</h4></div></a>
                    <div class="titlePost"><i>${dataPost.datePost}</i></div></a>
                    </div>
                </div>
                <div class="context">
...
$(document).ready(()=>{
  ...
  //Edit post student
  $('.editPostStudent').click(e =>{
    e.preventDefault()
    let id =$(e.target).data('id');
    $('.btnStudentEdit').attr('data-id',id)
    fetch('student/post',{
      method:'POST',
      headers:{
        'Content-Type':'application/x-www-form-urlencoded'
    ...
    .then(json=>{
      console.log(json)
      let d=json.data
      $('#titleStuE').val(d.titlePost)
      txtContentStuEdit =  CKEDITOR.instances['txtContentStuEdit'].setData(d.contextPost)
    })
    .catch(e=>console.log(e))
    $('#confirm-editStu-dialog').modal('show')
    CKEDITOR.replace('txtContentStuEdit')
  })
  
  $('.btnStudentEdit').click(e =>{
    let idStuE = e.target.dataset.id;
    let titleStuE = $('#titleStuE').val()
    let txtContentStuEdit =  CKEDITOR.instances['txtContentStuEdit'].getData()
    let data ={
      id:idStuE,
      titlePost:titleStuE,
      contextPost:txtContentStuEdit,
    }
    if(titleStuE===''|| txtContentStuEdit===''){
      $('#errorStuE').removeAttr('style')
      $('#errorStuE').text("Vui lòng nhập đầy đủ thông tin")
    }
    else{
      fetch('student/post/edit',{
        method:'POST',
        body:JSON.stringify(data)
      })
      .then(res=>res.json())
      .then(json=>{
        if(json.code === 1){
          $('#confirm-editStu-dialog').modal('hide')
        }else{
          $('#errorStuE').removeAttr('style')
          $('#errorStuE').text("")
          $('#errorStuE').text(json.message)
        }
      })
      .catch(e=>console.log(e))
    }
  ...
  $('.delPostStudent').click(e =>{
    e.preventDefault()
    let id = $(e.target).data('id')
    console.log(id)
    $('#btn-deleteStu-confirmed').attr('data-id',id)
    $('#confirm-deleteStu-dialog').modal('show')
  })

  $('#btn-deleteStu-confirmed').click(e =>{
    let id = e.target.dataset.id
    fetch('student/post/delete/'+id,{method:'POST'})
    .then(res=>res.json())
    .then(json => {
      if(json.code === 0){
        $('#confirm-deleteStu-dialog').modal('hide')
        window.location.reload()
        console.log(json.message)
      }else{
        $('#errorStuD').removeAttr('style')
        $('#errorStuD').text("")
        $('#errorStuD').text(json.message)
      }
